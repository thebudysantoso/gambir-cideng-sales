...
async function loadStats() {
  try {
    ...
    document.getElementById('statTotal').textContent = s.totalLeads;
    document.getElementById('statVisited').textContent = s.visited;
    document.getElementById('statNotVisited').textContent = s.notVisited || (s.totalLeads - s.visited);
...
async function loadFilters() {
  try {
    const r = await fetch(`${API}/leads?limit=999`);
...
function getLeadStatus(l) {
  if (l.pipeline_stage?.match(/Success|Deal|Converted/i)) return { cls: 'badge-success', text: 'SUKSES', color: '#137333' };
  if (l.sales_signal === 'DROP' || l.pipeline_stage?.match(/Drop|Lost/i)) return { cls: 'badge-drop', text: 'DROP', color: '#ea4335' };
  if (l.visit_status && (l.sales_signal === 'HOT' || l.sales_signal === 'WARM' || l.next_action)) return { cls: 'badge-need-actions', text: 'BUTUH TINDAKAN', color: '#ea4335' };
  if (l.visit_status) return { cls: 'badge-visited', text: 'DIKUNJUNGI', color: '#34a853' };
  const mobile = l.phone_primary?.match(/^(08|628|\+628)/);
  if (mobile) return { cls: 'badge-wa-available', text: 'BISA WA', color: '#4285f4' };
  return { cls: 'badge-not-visited', text: 'BELUM', color: '#9aa0a6' };
}
...
function renderLeads(leads) {
  ...
  el.innerHTML = leads.map(l => {
    ...
    return `
      <div class="lead-card" onclick="openLead('${l.lead_id}')">
...
function initMap() {
  if (map) return;
  map = L.map('map').setView([-6.18, 106.83], 14);
...
function renderMapMarkers() {
  ...
  allLeads.forEach(l => {
    ...
    const marker = L.circleMarker([l.lat, l.lng], {
      radius: l.sales_signal === 'HOT' ? 10 : 6,
      fillColor: color,
      color: '#fff',
      weight: 2,
      opacity: 1,
